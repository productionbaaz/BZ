...
function MainApp() {
  const { user, activeView, registry, accessibleSheetIds, sharePermissionFor, toggleBreak } = usePortal();
  ...
  async function handleToggleBreak() {
    setBreakBusy(true);
    try { await toggleBreak(); }
    catch (err) { alert('Could not update break status: ' + err.message); }
      ...
      <div className="main">
        <div className="topbar">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            {isEmployee && (
              <button
                className={user.onBreak ? 'btn-outline' : 'btn-gold'}
                style={user.onBreak ? { borderColor: 'var(--red)', color: 'var(--red)' } : undefined}
                disabled={breakBusy}
                onClick={handleToggleBreak}
              >
                {user.onBreak ? '⏸ End Break' : '☕ Take a Break'}
              </button>
            )}
