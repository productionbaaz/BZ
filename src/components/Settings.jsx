          ...
          return (
            <div key={entry.id} style={{ padding: '12px 14px', border: '1px solid var(--line)', borderRadius: 8, marginBottom: 8, background: 'var(--panel-2)' }}>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--muted-2)', marginTop: 6 }}>
                ...
                {entry.publicAccess && <span> · Public link: {entry.publicAccess}</span>}
              </div>
              {entry.id !== 'daily-report' && entry.id !== 'attendance' && (
                <div style={{ fontSize: 11, color: 'var(--muted-2)', marginTop: 4 }}>
                  Open this sheet and click its "Share" button to manage email/public access.
                </div>
              )}
