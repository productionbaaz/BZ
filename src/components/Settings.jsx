...
export default function Settings() {
  const {
    ...
    createSheet, deleteSheet, assignSheet, shareSheet, unshareSheet, saveBankDetails, setActiveView
  } = usePortal();
        ...
        {registry.map((entry) => {
          ...
          return (
            <div key={entry.id} style={{ padding: '12px 14px', border: '1px solid var(--line)', borderRadius: 8, marginBottom: 8, background: 'var(--panel-2)' }}>
              ...
              {openSharePanel === entry.id && (
                <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--line)' }}>
                  <p style={{ fontSize: 11.5, color: 'var(--muted-2)', margin: '0 0 8px' }}>Share with anyone by exact email — they don't need an employee account, just a free guest sign-up on the Employee Login page.</p>
