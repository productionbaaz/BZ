...
const ATTENDANCE_ACTION_LIST = ['Check In', 'Check Out', 'Break Start', 'Break End'];
...
function publicUser(uidVal, data) {
  return { id: uidVal, name: data.name, email: data.email, role: data.role, profilePic: data.profilePic || '', onBreak: !!data.onBreak };
}
...
/* Creates any missing default sheets AND patches in fields that got
   ...
   (sharedWith, publicAccess, assignedEmployees). Runs on every manager
   login — cheap and fully idempotent. Patches EVERY sheet actually in
   Firestore (not just the 10 known defaults), in case any custom sheet
   from earlier testing is also missing these fields — any one sheet
   missing them breaks the ENTIRE list read for every employee, since
   the permission check runs across the whole collection at once. */
async function ensureSeeded() {
  // 1) make sure all 10 default sheets exist at all
  const results = await Promise.allSettled(Object.keys(SHEET_DEFS).map(async (id) => {
  ...
  results.forEach((r) => { if (r.status === 'rejected') console.error('ensureSeeded: create step failed:', r.reason); });

  // 2) patch EVERY sheet that actually exists, whatever its origin
  const allSheets = await getDocs(collection(db, 'sheets'));
  const patchResults = await Promise.allSettled(allSheets.docs.map(async (d) => {
    const data = d.data();
    const patch = {};
    ...
    if (Object.keys(patch).length) await setDoc(doc(db, 'sheets', d.id), patch, { merge: true });
  }));
  patchResults.forEach((r) => { if (r.status === 'rejected') console.error('ensureSeeded: patch step failed:', r.reason); });
...
export const firebasedb = {
  ...
  /* Toggles an employee's break status — logs "Break Start"/"Break End"
     into Attendance (same sheet as check-in/out, same _ownerUid rules,
     no extra permission needed) and flips a simple onBreak flag on
     their own profile so the button knows what to show next. */
  async toggleBreak() {
    const u = auth.currentUser;
    if (!u) throw new Error('Not logged in.');
    const data = await getUserDoc(u.uid);
    if (!data || data.role !== 'employee') throw new Error('Only employees can log breaks.');
    const startingBreak = !data.onBreak;
    await logAttendance(u.uid, data.name, startingBreak ? 'Break Start' : 'Break End');
    await updateDoc(doc(db, 'users', u.uid), { onBreak: startingBreak });
    return { onBreak: startingBreak };
  },
