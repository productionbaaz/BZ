...
export const firebasedb = {
  ...
  async employeeLogin({ email, password }) {
    ...
    await logAttendance(cred.user.uid, data.name, 'Check In');
    return { user: publicUser(cred.user.uid, data) };
  },
  ...
  async getRegistry() {
    ...
    return { registry: snap.docs.map((d) => ({ id: d.id, name: d.data().name, icon: d.data().icon, assignedEmployees: d.data().assignedEmployees || [], sharedWith: d.data().sharedWith || {} })) };
  },

  async createSheet({ name, assignedEmployees, columns }) {
    const id = 'sheet-' + normLabel(name).slice(0, 20) + '-' + uid().slice(0, 4);
    await setDoc(doc(db, 'sheets', id), { name, icon: 'box', assignedEmployees: assignedEmployees || [], sharedWith: {}, columns });
    return { id };
  },
  ...
  async getSheet({ sheetId }) {
    ...
    const columns = sheetSnap.exists() ? sheetSnap.data().columns : [];
    const u = auth.currentUser;
